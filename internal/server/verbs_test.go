...
func TestMoveVerbsReorderWithoutResizing(t *testing.T) {
	...
	s.handleClientMsg(context.Background(), &protocol.ClientEnvelope{Payload: &protocol.ClientEnvelope_Verb{Verb: &protocol.MsgVerb{Verb: protocol.VerbType_VERB_MOVE_LEFT}}})
	...
	s.handleClientMsg(context.Background(), &protocol.ClientEnvelope{Payload: &protocol.ClientEnvelope_Verb{Verb: &protocol.MsgVerb{Verb: protocol.VerbType_VERB_MOVE_RIGHT}}})
...
func TestFocusLastVerbFlipsBack(t *testing.T) {
	...
	s.handleClientMsg(ctx, &protocol.ClientEnvelope{Payload: &protocol.ClientEnvelope_FocusPane{FocusPane: &protocol.MsgFocusPane{PaneId: int32(order[0])}}})
	s.handleClientMsg(ctx, &protocol.ClientEnvelope{Payload: &protocol.ClientEnvelope_FocusPane{FocusPane: &protocol.MsgFocusPane{PaneId: int32(order[2])}}})

	s.handleClientMsg(ctx, &protocol.ClientEnvelope{Payload: &protocol.ClientEnvelope_Verb{Verb: &protocol.MsgVerb{Verb: protocol.VerbType_VERB_FOCUS_LAST}}})
